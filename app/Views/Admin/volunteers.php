        ...
        <div class="tab-content">
            ...
            <div id="tab3" class="tab-pane fade">
                <div class="row">
                    <div class="col-md-12">
                        <div class="white-box">                    
                            <h2><b>تعديل متطوع</b></h2>
                            <hr>
                            <form id="volunteerEdit">
                                <div class="row">
                                    <div class="form-group">
                                        <?php foreach ($entityData as $field => $attributes): ?>
                                            <div <?php if($language == 'en'){ echo 'style="float: left !important"';};?>  class="<?php echo $attributes['class_id'] ;?>" id="volunteer_<?= $field ?>_X">
                                                <?php
                                                ...
                                                switch ($attributes['type']) {
                                                    ...
                                                    case 'file':
                                                    case 'phone':
                                                        echo '<input type="' . $attributes['type'] . '" class="form-control" id="volunteer_' . $attributes['id'] . '_X" name="' . $field . '" placeholder="' . $attributes['placeholder'] . '" ' . ($attributes['required'] ? 'required' : '') . ' ' . (isset($attributes['accept']) ? 'accept="' . $attributes['accept'] . '"' : '') . '>';
                                                        break;
                                                    case 'select':
                                                        echo '<select class="form-control" id="volunteer_' . $attributes['id'] . '_X" name="' . $field . '" ' . ($attributes['required'] ? 'required' : '') . '>';
                                                        foreach ($attributes['options'] as $value => $label) {
                                                            echo '<option value="' . $value . '">' . $label . '</option>';
                                                        }
                                                        echo '</select>';
                                                        break;
                                                    case 'textarea':
                                                        echo '<textarea class="form-control" id="volunteer_' . $attributes['id'] . '_X" name="' . $field . '" placeholder="' . $attributes['placeholder'] . '" ' . ($attributes['required'] ? 'required' : '') . '></textarea>';
                                                        break;
                                                    default:
                                                        echo '<input type="text" class="form-control" id="volunteer_' . $attributes['id'] . '_X" name="' . $field . '" placeholder="' . $attributes['placeholder'] . '" ' . ($attributes['required'] ? 'required' : '') . '>';
                                                }
                                                ?>
                                            </div>
                                        <?php endforeach; ?>
    ...
    <script>
        $(document).ready(function () 
        {
            ...
            $('.btn-delete').on('click', function () {            
                ...
                if (!confirm(`هل أنت متأكد أنك تريد حذف هذا المتطوع؟`)) return;
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status === 'success') {
                            ...
                            window.showNotification('success', 'تم حذف المتطوع بنجاح');
                        }
            ...
            $(document).on('submit', '#volunteerEdit', function (e) {
                e.preventDefault();
            ...
            window.openAddModal = function() {
                ...
                $('#modalTitle').text('إضافة متطوع جديد');
                $('#modalBody').html(formHtml);
                $('#managementModal').fadeIn().css('display', 'flex');
                $('body').addClass('modal-open');
                
                $('#volunteerAdd').on('submit', function(e) {
                    e.preventDefault();
                    let formData = new FormData(this);
                            ...
                            $.ajax({
                                ...
                                success: function(res) { 
                                    if (res.status === 'success') {
                                        ...
                                        $('#managementModal').fadeOut();
                                        window.showNotification('success', 'تمت إضافة المتطوع بنجاح');
        ...
        function editEntity(id) {
            ...
            $('#modalTitle').text('تعديل بيانات المتطوع');
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status === 'success') {
                        // Populate form inside the modal
                        $.each(response.data, function (field, value) {
                            const element = $(`#modalBody #volunteer_${field}_X`);
                            if (element.length) {
