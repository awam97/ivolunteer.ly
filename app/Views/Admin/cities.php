        ...
        <div class="tab-content">
            <div id="tab1" class="tab-pane fade in active">
                <div class="row grid-container" id="gridContainerOne">
                    <?php foreach ($entities as $entity): ?>
                        <div class="city-item entity-card-wrapper" data-id="<?= $entity->id; ?>">
                            <div class="white-box table-box-items entity-card-content">  
                                ...
                                <div class="entity-header-row">
                                    ...
                                    <div class="entity-info">                                               
                                        ...
                                        <p class="entity-meta text-muted"><i class="fa-solid fa-map-pin"></i> منطقة جغرافية</p>
                                    </div>
                                </div>
            ...
            <div id="tab3" class="tab-pane fade">
                <div class="row">
                    <div class="col-md-12">
                        <div class="white-box">                    
                            <h2><b>تعديل مدينة</b></h2>
                            <hr>
                            <form id="cityEdit">
                                <div class="row">
                                    <div class="form-group">
                                        <?php foreach ($entityData as $field => $attributes): ?>
                                            <div class="<?php echo $attributes['class_id'] ;?>" id="city_<?= $field ?>_X">
                                                <?php
                                                echo '<label for="' . $attributes['id'] . '_X"><b>' . $attributes['placeholder'] . '</b></label>';
                                                echo '<input type="text" class="form-control" id="city_' . $attributes['id'] . '_X" name="' . $field . '" placeholder="' . $attributes['placeholder'] . '" ' . ($attributes['required'] ? 'required' : '') . '>';
                                                ?>
                                            </div>
                                        <?php endforeach; ?>
    ...
    <script>
        $(document).ready(function () 
        {
            ...
            $('.btn-delete').on('click', function () {            
                ...
                if (!confirm(`هل أنت متأكد أنك تريد حذف هذه المدينة؟`)) return;
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status === 'success') {
                            ...
                            window.showNotification('success', 'تم حذف المدينة بنجاح');
                        }
            ...
            $('#bulkDeleteBtn').on('click', function () {
                ...
                const confirmMsg = selectAllMode ? 'هل أنت متأكد أنك تريد حذف جميع السجلات؟' : 'هل أنت متأكد أنك تريد حذف المدن المختارة؟';
            ...
            $(document).on('submit', '#cityEdit', function (e) {
                e.preventDefault();
                    ...
                    $.ajax({
                        ...
                        success: function(res) { 
                            if (res.status === 'success') {
                                ...
                                $('#managementModal').fadeOut();
                                window.showNotification('success', 'تمت إضافة المدينة بنجاح');
                ...
                success: function (response) {
                    if (response.status === 'success') {
                        ...
                        $.each(response.data, function (field, value) {
                            const element = $(`#modalBody #city_${field}_X`);
