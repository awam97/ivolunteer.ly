<?php
    ...
    $image_url = $fileModel->get_image_url($entityName, $volunteer_id);
                        ...
                        <?php foreach ($entityData as $field => $attributes): ?>
                            <?php if ($attributes['type'] === 'password'): ?>
                                <div class="form-group">
                                    <label><?= $attributes['placeholder'] ?></label>
                                    <input type="password" class="form-control" id="<?= $entityName .'_' .$attributes['id'].'_X' ?>" name="<?= $field ?>" placeholder="<?= $attributes['placeholder'] ?>">
                                </div>
                                <div class="form-group">
                                    <label>تأكيد كلمة المرور الجديدة</label>
                                    <input type="password" class="form-control" id="repeat_password" placeholder="تأكيد كلمة المرور الجديدة">
                                </div>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </div>
...
<script>
    ...
    {        
        ...
        function updateProfile(formData, btn) {
            ...
            $.ajax({
                url: '<?= base_url('Volunteer/update_post_entity') ?>',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        // --- Data Fetching & Sync ---
        $.ajax({
            url: `<?= base_url("Volunteer/data_grap") ?>`,
            type: 'POST',
